...
int setup_conv_opts(conv_opts &opts, PCS eps, PCS upsampfac, int kerevalmeth){
    ...
    if (kw>MAX_KERNEL_WIDTH) {         // clip to match allocated arrays
    ...
    if (kw==2) betaoverns = 2.20;  // some small-width tweaks...
    if (kw==3) betaoverns = 2.26;
    if (kw==4) betaoverns = 2.38;
    ...
    case 0:
    {
      if(plan->opts.gpu_sort){
		...
		case 2:
			{
				int numbins[2];
				numbins[0] = ceil((PCS) nf1/d_plan->opts.gpu_binsizex);
				numbins[1] = ceil((PCS) nf2/d_plan->opts.gpu_binsizey);
